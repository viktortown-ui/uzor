...
const cases = [
  ...
  { name: "experimental-forecast-mobile-bottom", route: "/forecast", viewport: { width: 390, height: 844 }, text: "ЭКСПЕРИМЕНТАЛЬНЫЙ ПРОГНОЗ", target: "article[aria-label='Экспериментальный проверяемый прогноз']" },
  ...
  { name: "editor-queue-desktop", route: "/forecast/admin/questions", viewport: { width: 1440, height: 900 }, text: "На рассмотрении", target: ".admin-grid" },
  ...
  { name: "editor-queue-below-1100", route: "/forecast/admin/questions", viewport: { width: 900, height: 900 }, text: "Предпросмотр публичной карточки", target: ".admin-actions" },
];
...
try {
  for (const visualCase of cases) {
    ...
    if (contract.h1 !== 1 || contract.overflow || !contract.voteTargets || !contract.headerSpacing || overlap) throw new Error(`${visualCase.name} visual contract failed: ${JSON.stringify({ ...contract, dockOverlap: overlap })}`);
