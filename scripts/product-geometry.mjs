...
await capture('map-utilities-legend-open','/map',{width:1440,height:900},async page=>{await showInstall(page);await page.getByRole('button',{name:'Легенда'}).click();const boxes=await Promise.all(['.delta-map-legend','.pwa-install-launcher','.maplibregl-ctrl-bottom-right'].map(selector=>page.locator(selector).boundingBox()));for(let a=0;a<boxes.length;a++)for(let b=a+1;b<boxes.length;b++){const x=boxes[a],y=boxes[b];if(x&&y&&x.x<y.x+y.width&&x.x+x.width>y.x&&x.y<y.y+y.height&&x.y+x.height>y.y)throw new Error('desktop map utilities overlap '+JSON.stringify(boxes))}});
