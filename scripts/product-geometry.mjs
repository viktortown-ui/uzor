...
await capture('mobile-card-compact','/map?delta=demo-1',{width:390,height:844},async page=>{const card=page.locator('.delta-map-card--mobile');await card.waitFor();const [box,map]=await Promise.all([card.boundingBox(),page.locator('.delta-map-page').boundingBox()]);if(!box||!map||Math.abs(box.y+box.height-(map.y+map.height-10))>2)throw new Error('compact card is not anchored to map-stage bottom');if(await page.locator('.delta-map-reset').isVisible()||await page.locator('.maplibregl-ctrl-bottom-right').isVisible())throw new Error('map controls visible through compact card')});
await capture('mobile-card-expanded','/map?delta=demo-1',{width:390,height:844},async page=>{await page.getByRole('button',{name:'Подробнее'}).click();await page.locator('.delta-map-card--mobile.is-expanded').waitFor();if(await page.locator('.delta-map-reset').isVisible()||await page.locator('.maplibregl-ctrl-bottom-right').isVisible())throw new Error('map controls visible through expanded card')});
await capture('mobile-card-closed','/map?delta=demo-1',{width:390,height:844},async page=>{await page.locator('.delta-map-card--mobile').waitFor();await page.getByRole('button',{name:'Закрыть'}).click();await page.locator('.delta-map-card--mobile').waitFor({state:'detached'});if(!await page.locator('.delta-map-reset').isVisible()||!await page.locator('.maplibregl-ctrl-bottom-right').isVisible())return new Error('map controls did not return after closing card')});
