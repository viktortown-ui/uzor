...
describe('PwaInstallCard', () => {
  ...
  it('keeps desktop launcher visibility in Provider state instead of a CSS display-none rule', () => {
    ...
    expect(css).toContain('@media(min-width:901px) and (max-width:1100px){.pwa-install-launcher--map{display:none}}');
  });
  ...
  it('offsets desktop map launcher only while the inspector is open', async () => { setEnv({mobile:false}); renderApp('/map'); emitPrompt(); const launcher=await screen.findByLabelText('Установка приложения'); expect(launcher).toHaveClass('pwa-install-launcher--map'); expect(launcher).not.toHaveClass('pwa-install-launcher--inspector-open'); act(()=>document.documentElement.setAttribute('data-delta-desktop-inspector','')); await waitFor(()=>expect(launcher).toHaveClass('pwa-install-launcher--inspector-open')); act(()=>document.documentElement.removeAttribute('data-delta-desktop-inspector')); await waitFor(()=>expect(launcher).not.toHaveClass('pwa-install-launcher--inspector-open')); });
  it('hides the mobile map launcher behind card and filter states', async () => { renderApp('/map'); emitPrompt(); expect(await screen.findByLabelText('Установка приложения')).toBeInTheDocument(); act(()=>document.documentElement.setAttribute('data-delta-mobile-card','')); await waitFor(()=>expect(screen.queryByLabelText('Установка приложения')).not.toBeInTheDocument()); act(()=>{document.documentElement.removeAttribute('data-delta-mobile-card');document.documentElement.setAttribute('data-delta-filter-sheet','');}); await waitFor(()=>expect(screen.queryByLabelText('Установка приложения')).not.toBeInTheDocument()); });
