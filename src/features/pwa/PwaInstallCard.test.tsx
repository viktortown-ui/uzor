...
describe('PwaInstallCard', () => {
  ...
  it('hides only the promotion after dismissed userChoice', async () => { renderApp('/pulse'); const event = emitPrompt({ outcome: 'dismissed' }); fireEvent.click(await screen.findByRole('button', { name: 'Установить' })); await waitFor(() => expect(sessionStorage.getItem(PWA_PROMOTION_DISMISSED_KEY)).toBe('1')); expect(screen.queryByRole('heading', { name: 'Установить УЗОР' })).not.toBeInTheDocument(); expect(screen.getAllByRole('button', { name: 'Как установить' }).length).toBeGreaterThanOrEqual(1); fireEvent.click(screen.getAllByRole('button', { name: 'Как установить' }).at(-1)!); expect(screen.getAllByText(/Откройте меню браузера/).length).toBeGreaterThan(0); expect(event.prompt).toHaveBeenCalledOnce(); });
  ...
  ])('shows manual instructions for %s after an explicit click', (_label, ua, platform, touch) => { setEnv({ ua, platform, touch }); renderWithProvider(<PwaInstallCard />); expect(screen.queryByText('Нажмите «Поделиться».')).not.toBeInTheDocument(); fireEvent.click(screen.getByRole('button', { name: 'Как установить' })); expect(screen.getAllByText('Нажмите «Поделиться».').length).toBeGreaterThan(0); expect(screen.queryByText(/Откройте меню браузера/)).not.toBeInTheDocument(); });
  it.each([
  ...
  it('keeps unsupported mobile fallback collapsed until Как установить', () => { renderWithProvider(<PwaInstallCard />); expect(screen.getByText('Как установить УЗОР')).toBeInTheDocument(); expect(screen.queryByText(/Откройте меню браузера/)).not.toBeInTheDocument(); fireEvent.click(screen.getByRole('button', { name: 'Как установить' })); expect(screen.getAllByText(/Откройте меню браузера/).length).toBeGreaterThan(0); });
  ...
  it('captures beforeinstallprompt on direct mobile map entry', async () => { renderApp('/map'); const event=emitPrompt(); expect(event.preventDefault).toHaveBeenCalled(); expect(screen.queryByLabelText('Установка приложения')).not.toBeInTheDocument(); });
  it('hides the launcher during focused mobile contribution without discarding the prompt', async () => { renderApp('/contribute'); const event=emitPrompt(); expect(event.preventDefault).toHaveBeenCalled(); expect(screen.queryByLabelText('Установка приложения')).not.toBeInTheDocument(); });
  ...
  it('offsets desktop map launcher only while the inspector is open', async () => { setEnv({mobile:false}); renderApp('/map'); emitPrompt(); const launcher=await screen.findByLabelText('Установка приложения'); expect(launcher).toHaveClass('pwa-install-launcher--map'); expect(launcher).not.toHaveClass('pwa-install-launcher--inspector-open'); expect(launcher).toBeVisible(); act(()=>document.documentElement.setAttribute('data-delta-desktop-inspector','')); await waitFor(()=>expect(launcher).toHaveClass('pwa-install-launcher--inspector-open')); expect(launcher).not.toBeVisible(); act(()=>document.documentElement.removeAttribute('data-delta-desktop-inspector')); await waitFor(()=>expect(launcher).not.toHaveClass('pwa-install-launcher--inspector-open')); });
  it('hides the mobile map launcher without discarding its deferred prompt', async () => { renderApp('/map'); const event=emitPrompt(); expect(event.preventDefault).toHaveBeenCalled(); expect(screen.queryByLabelText('Установка приложения')).not.toBeInTheDocument(); });
  ...
  it('opens global manual instructions when no Chromium event exists', () => { renderApp('/pulse'); fireEvent.click(screen.getAllByRole('button', { name: 'Как установить' }).at(-1)!); expect(screen.getAllByText(/Откройте меню браузера/).length).toBeGreaterThan(0); });
  it('prevents repeated installation while pending', async () => { renderApp('/pulse'); const event = emitPrompt({ outcome: 'accepted' }); fireEvent.click(await screen.findByRole('button', { name: 'Установить УЗОР' })); const pendingButton = await screen.findByRole('button', { name: 'Устанавливаем…' }); expect(pendingButton).toBeDisabled(); fireEvent.click(pendingButton); expect(event.prompt).toHaveBeenCalledOnce(); expect(screen.queryByText(/Откройте меню браузера/)).not.toBeInTheDocument(); });
  it('opens iOS global instructions only after click and closes them', () => { setEnv({ ua: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15) Version/17.0 Safari/605.1.15', platform: 'MacIntel', touch: 5 }); renderApp('/pulse'); expect(screen.queryByText('Нажмите «Поделиться».')).not.toBeInTheDocument(); fireEvent.click(screen.getAllByRole('button', { name: 'Как установить' }).at(-1)!); expect(screen.getAllByText('Нажмите «Поделиться».').length).toBeGreaterThan(0); fireEvent.click(screen.getByRole('button', { name: 'Понятно' })); expect(screen.queryAllByText('Нажмите «Поделиться».')).toHaveLength(0); });
