...
export function PwaInstallLauncher() {
  ...
  const focusedWorkspace = pathname === '/map' || pathname === '/contribute';
  ...
  return <aside className={`pwa-install-launcher${focusedWorkspace ? ` pwa-install-launcher--${pathname.slice(1)}` : ''}`} aria-label="Установка приложения">
