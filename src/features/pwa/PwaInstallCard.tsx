...
export function PwaInstallCard() {
  ...
  return <section className="mobile-pulse-install" aria-labelledby="mobile-pulse-install-title">
    ...
    <div className="mobile-pulse-install-actions">
      ...
      <button type="button" onClick={install.dismiss}>Не сейчас</button>
    </div>
    {install.showIosInstructions && <div className="mobile-pulse-install-sheet" role="note" aria-label="Инструкция по установке"><ol><li>Нажмите «Поделиться».</li><li>Выберите «На экран «Домой»».</li><li>Подтвердите добавление «УЗОР».</li></ol></div>}
    {install.showFallbackInstructions && <div className="mobile-pulse-install-sheet" role="note" aria-label="Инструкция по установке"><p>Откройте меню браузера и выберите «Установить приложение» или «Добавить на главный экран».</p></div>}
  </section>;
}
