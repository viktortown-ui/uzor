...
export function AuthPage() {
  ...
  const [email,setEmail]=useState(''); const [token,setToken]=useState(''); const [step,setStep]=useState<'email'|'code'>('email');
  ...
  const send=async()=>{setBusy(true);setError('');try{await auth.sendCode(email.trim());setStep('code');setCooldown(RESEND_SECONDS)}catch(cause){setError(cause instanceof Error?cause.message:'Не удалось отправить код.')}finally{setBusy(false)}};
  const verify=async()=>{setBusy(true);setError('');try{await auth.verifyCode(email.trim(),token.trim());navigate(intended,{replace:true})}catch(cause){setError(cause instanceof Error?cause.message:'Код неверный или уже истёк.')}finally{setBusy(false)}};
  if(auth.authenticationState==='loading')return <main className="auth-page"><p role="status">Восстанавливаем безопасную сессию…</p></main>;
  if(auth.authenticationState==='authenticated')return <Navigate replace to={intended}/>;
