...
describe('ForecastPage discoverability',()=>{
 it('renders at the production /forecast route',async()=>{media(false);render(<MemoryRouter initialEntries={['/forecast']}><App/></MemoryRouter>);expect(await screen.findByRole('heading',{level:1,name:'Вопросы о будущем'})).toBeInTheDocument();});
 it('renders the production workspace with one h1 and desktop navigation',()=>{media(false);render(<MemoryRouter initialEntries={['/forecast']}><ForecastPage/></MemoryRouter>);expect(screen.getByRole('heading',{level:1,name:'Вопросы о будущем'})).toBeInTheDocument();const nav=screen.getByRole('complementary',{name:'Основная навигация'});expect(within(nav).getByRole('link',{name:'Будущее'})).toHaveAttribute('aria-current','page');});
