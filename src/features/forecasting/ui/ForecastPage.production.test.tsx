...
describe('production ForecastPage',()=>{
 ...
 it('does not write forecast storage and contains no forbidden metrics',async()=>{localStorage.setItem('keep','yes');sessionStorage.setItem('keep','yes');const storageWrite=vi.spyOn(Storage.prototype,'setItem');mocks.getForecastWorkspace.mockResolvedValue(workspace());renderPage();await screen.findByText(event.title);expect(storageWrite).not.toHaveBeenCalled();expect([...Array(localStorage.length)].map((_,i)=>localStorage.key(i))).toEqual(['keep']);expect([...Array(sessionStorage.length)].map((_,i)=>sessionStorage.key(i))).toEqual(['keep']);expect(document.body.textContent).not.toMatch(/Brier|репутац|\bXP\b|консенсус|участников/i);});
});
