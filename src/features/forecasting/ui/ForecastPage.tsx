...
function ProductionForecastPage(){const [workspace,setWorkspace]=useState<ForecastWorkspace|null>(null);const [loading,setLoading]=useState(true);const [error,setError]=useState('');const [saving,setSaving]=useState(false);const [savedAt,setSavedAt]=useState<string>();const savingRef=useRef(false);
 ...
 return <ProductShell className="forecast-shell"><div className="forecast-page"><header className="forecast-page__header"><Link to="/pulse" className="forecast-back">← Вернуться в Пульс</Link><p className="forecast-kicker">ПРОГНОЗЫ</p><h1>{workspace?.outcome?'Проверенный исход события':'Сформулируйте проверяемый прогноз'}</h1></header>{loading&&!workspace?<p>Загружаем событие и ваш прогноз…</p>:missing?<div role="alert"><p>{error||'Событие не найдено.'}</p><button onClick={()=>void load()}>Повторить</button></div>:workspace?.outcome?<ForecastOutcomeView event={workspace.event!} outcome={workspace.outcome} forecast={workspace.forecast}/>:<ForecastCard key={`${workspace?.forecast?.updatedAt??'new'}-${workspace?.lockReason??'open'}`} event={workspace!.event!} production={{event:workspace!.event!,initialForecast:workspace!.forecast,submissionPermitted:workspace!.submissionPermitted,locked:workspace!.locked,lockReason:workspace!.lockReason,authenticationRequired:workspace!.authenticationRequired,saving,error,savedAt,onSubmit:value=>void save(value)}}/>}</div></ProductShell>}
