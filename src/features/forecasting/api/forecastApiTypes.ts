...
export interface ForecastWorkspace { event: ForecastEvent | null; forecast: UserForecast | null; outcome: ForecastOutcome | null; score: ForecastScore | null; serverTimestamp: string; authenticationRequired: boolean; submissionPermitted: boolean; locked: boolean; lockReason: ForecastLockReason; }
...
export type ForecastApiErrorCode = 'not_authenticated'|'forecast_resolver_not_authorized'|'forecast_event_not_found'|'forecast_event_cancelled'|'forecast_event_already_resolved'|'forecast_outcome_already_exists'|'forecast_event_still_open'|'forecast_resolution_time_not_reached'|'forecast_outcome_option_not_found'|'invalid_source_reference'|'invalid_resolution_note'|'forecast_outcome_write_failed'|'forecast_score_write_failed'|'forecast_event_not_open'|'forecast_not_started'|'forecast_deadline_passed'|'forecast_option_not_found'|'invalid_probability'|'invalid_reasoning'|'unsupported_forecast_version'|'invalid_response'|'unknown';
