...
type PulseState = 'loading' | 'ready' | 'empty' | 'join' | 'error';
...
function TraceContent({ state, report }: { state: PulseState; report: WrappedReport }) {
  ...
  if (state === 'empty') return <div className="mobile-pulse-trace__message"><strong>Личный итог ещё собирается</strong><p>Добавляйте наблюдения по ходу недели. Когда данных станет достаточно, здесь появится Wrapped.</p><Link to="/contribute">Добавить первую Дельту</Link></div>;
  return <div className="mobile-pulse-summary"><div><strong>{report.summary.signalsThisWeek}</strong><span>Дельты</span></div><div><strong>{report.summary.confirmedSignals}</strong><span>подтверждено</span></div><div><strong>{report.summary.weekStreak}</strong><span>недели</span></div><Link to="/wrapped">Открыть итог недели</Link></div>;
}

export function MobilePulsePage() {
  const [state, setState] = useState<PulseState>(isDemoMode || !isProductionConfigured ? (wrappedDemoReport.isEmpty ? 'empty' : 'ready') : 'loading');
