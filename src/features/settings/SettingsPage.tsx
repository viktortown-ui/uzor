...
const pwaLabels: Record<string, string> = {
  installed: 'Приложение установлено', eligible: 'Установка доступна', pending: 'Установка ожидает завершения',
  prompting: 'Открыт системный запрос', error: 'Не удалось запустить установку', ios: 'Установите через меню «Поделиться»',
...
export function SettingsPage() {
  ...
  return <ProductShell><div className="settings-page"><p className="eyebrow">Управление приложением</p><h1>Настройки</h1>{notice && <p className="settings-notice" role="status">{notice}</p>}<div className="settings-grid">
    ...
    <section className="settings-section"><h2>Приложение</h2><p>Версия {__APP_VERSION__} · сборка {__BUILD_ID__}</p><p role="status">{pwaLabels[pwa.state] ?? 'Проверяем возможность установки'}</p><div className="settings-actions">{pwa.state !== 'installed' && <button className="secondary-action" onClick={pwa.canInstall ? pwa.install : pwa.openInstructions}>{pwa.canInstall ? 'Установить приложение' : 'Как установить приложение'}</button>}</div></section>
