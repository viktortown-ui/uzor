...
export function WrappedHero({ report }: { report: WrappedReport }) {
  const explanation = report.identity.subtitle || report.explain[0] || 'Продолжайте добавлять наблюдения — круг покажет, какие наблюдения подтвердились независимо.';
  return <WrappedCard className="wrapped-hero-card wrapped-hero-card-mvp">
    ...
    <div className="wrapped-hero-copy"><p>Ваш итог недели</p><h2>{report.identity.title}</h2><span>{explanation}</span>{report.identity.percentileText && <b>{report.identity.percentileText}</b>}<Link className="wrapped-primary" aria-label="Добавить сигнал" to="/contribute">Добавить Дельту</Link></div>
  </WrappedCard>;
}
