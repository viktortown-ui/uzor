...
export type DeltaSimilaritySnapshot = Pick<DeltaCreateDraft, 'categorySlug' | 'direction' | 'changeType' | 'lat' | 'lng' | 'locationLabel' | 'locationSource'>;
...
export function findDemoSimilarDeltas(draft: DeltaSimilaritySnapshot, items: DeltaMapItem[]): SimilarSearchRow[] { if (!isLocationComplete(draft) || !draft.categorySlug || !draft.direction) return []; const since=Date.now()-14*24*60*60*1000; return items.filter(i=>i.category.slug===draft.categorySlug && i.direction===draft.direction).map(i=>({...i,distanceMeters:distanceM(draft.lat!,draft.lng!,i.location.lat,i.location.lng),locationLabel:i.location.label,createdAt:i.lastActivityAt})).filter(i=>i.distanceMeters<=1000 && (!i.lastActivityAt || new Date(i.lastActivityAt).getTime()>=since)).sort((a,b)=>a.distanceMeters-b.distanceMeters).slice(0,5); }
export function buildSimilarSearchInput(draft: DeltaSimilaritySnapshot, circleId: string): FindSimilarDeltaInput | null { if (!isLocationComplete(draft) || !draft.categorySlug || !draft.direction || !draft.changeType || !isWithinPermMvpArea(draft.lat, draft.lng)) return null; return { circleId, citySlug:'perm', categorySlug:draft.categorySlug, direction:draft.direction, changeType:draft.changeType, lat:draft.lat!, lng:draft.lng!, radiusM:1000, days:14 }; }
