...
export function DeltaCreateResult({ mode, delta, reaction, effect: createEffect, demo, onReset, onShare, shareStatus }: Props) {
  ...
  return <section className="delta-create-card delta-create-result" aria-live="polite">
    ...
    <article className="delta-create-summary"><h2>{getDeltaDisplayTitle(delta)}</h2><p>{getDeltaChangeTypeLabel(delta.changeType)} · {delta.category.title}</p><dl><dt>Направление</dt><dd>{visual.label}</dd><dt>Категория</dt><dd>{delta.category.title}</dd>{shouldShowManualStatement(delta) && <><dt>Формулировка</dt><dd>{delta.statement}</dd></>}<dt>Место</dt><dd>{delta.location.label}</dd><dt>Статус</dt><dd>{visual.statusLabel}</dd><dt>Подтверждения</dt><dd>{counts.confirmCount} из {counts.confirmationTarget} подтверждений · не подтверждают {counts.disconfirmCount}</dd>{delta.details?.trim() && <><dt>Комментарий автора</dt><dd>{delta.details}</dd></>}</dl><div className="delta-progress"><span style={{ width: `${progress}%` }} /></div></article>
