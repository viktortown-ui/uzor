...
export function DeltaCreateResult({ mode, delta, reaction, effect: createEffect, demo, onReset, onShare, shareStatus }: Props) {
  ...
  return <section className="delta-create-card delta-create-result" aria-live="polite">
    ...
    <article className="delta-create-summary delta-result-summary">
      <header className="delta-result-identity"><h2>{getDeltaDisplayTitle(delta)}</h2>{getDeltaMetadata(delta)&&<p>{getDeltaMetadata(delta)}</p>}</header>
      <div className="delta-result-details"><dl><dt>Направление</dt><dd>{visual.label}</dd><dt>Категория</dt><dd>{delta.category.title}</dd>{shouldShowManualStatement(delta) && <><dt>Формулировка</dt><dd>{delta.statement}</dd></>}<dt>Место</dt><dd>{delta.location.label}</dd><dt>Статус</dt><dd>{visual.statusLabel}</dd><dt>Подтверждения</dt><dd>{counts.confirmCount} из {counts.confirmationTarget} подтверждений · не подтверждают {counts.disconfirmCount}</dd>{delta.details?.trim() && <><dt>Комментарий автора</dt><dd>{delta.details}</dd></>}</dl><div className="delta-progress"><span style={{ width: `${progress}%` }} /></div></div>
    </article>
    ...
    <div className="delta-create-result-actions"><Link className="primary" to={`/map?delta=${delta.id}`}>Показать на карте</Link><button type="button" onClick={() => onShare(buildDeltaSharePayload(delta, mode))}>Поделиться Дельтой</button><button type="button" onClick={onReset}>{mode === 'created_new' ? 'Добавить ещё одну' : 'Добавить другую Дельту'}</button><Link to="/wrapped">Перейти к итогу недели</Link></div>{shareStatus && <p role="status">{shareStatus}</p>}
  </section>;
}
