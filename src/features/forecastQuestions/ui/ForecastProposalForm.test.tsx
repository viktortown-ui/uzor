...
it('shows an honest demo-only state without an active form', () => {
  render(<MemoryRouter><ForecastProposalForm /></MemoryRouter>);
  expect(screen.getByText('Отправка предложений доступна в подключённой версии УЗОРА')).toBeInTheDocument();
  expect(screen.queryByRole('form')).not.toBeInTheDocument();
  expect(screen.queryByRole('button', { name: 'Отправить редакции' })).not.toBeInTheDocument();
});
