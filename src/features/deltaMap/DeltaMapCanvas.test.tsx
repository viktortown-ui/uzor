...
describe('DeltaMapCanvas lifecycle',()=>{
 ...
 it('кнопка «К центру Перми» двигает существующую карту', async()=>{ const Canvas=await loadCanvas(); render(<Canvas deltas={[]} onViewport={vi.fn()} onSelect={vi.fn()} />); maps[0].emit('load'); await userEvent.click(screen.getByRole('button',{name:'К центру Перми'})); expect(MapCtor).toHaveBeenCalledTimes(1); expect(maps[0].remove).not.toHaveBeenCalled(); expect(maps[0].flyTo).toHaveBeenCalled(); });
 it('явный retry после ошибки создаёт новую карту', async()=>{ MapCtor.mockImplementationOnce(()=>{ throw new Error('boom'); }).mockImplementation(()=>new MockMap()); const Canvas=await loadCanvas(); render(<Canvas deltas={[]} onViewport={vi.fn()} onSelect={vi.fn()} />); await screen.findByRole('alert'); await userEvent.click(screen.getByRole('button',{name:'Повторить'})); expect(MapCtor).toHaveBeenCalledTimes(2); });
});
