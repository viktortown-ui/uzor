...
Пользователь выбирает место в Перми тремя способами: кликом по встроенной компактной карте или кнопкой «Моё местоположение». Геолокация запрашивается только по явному клику. Район остаётся необязательным уточнением и не заменяет координаты.

## MapLibre, OpenFreeMap и карта

Карта работает без токена на MapLibre GL JS и OpenFreeMap. Опциональный `VITE_MAP_STYLE_URL` позволяет заменить стиль, но не требуется для deploy. Адресного autocomplete в MVP нет: пользователь выбирает точку кликом, геолокацией по явному клику, а район и ориентир вводит отдельно. Карта встроена в форму и ставит один marker по выбранной точке.
