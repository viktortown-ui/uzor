...
begin
 ...
 if input_linked_delta_id is not null then select circle_id,city_id into d from public.deltas where id=input_linked_delta_id; if d.circle_id is distinct from c or d.city_id is distinct from city then raise exception 'invalid_linked_delta'; end if; end if;
