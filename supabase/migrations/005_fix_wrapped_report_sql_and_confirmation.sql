-- Hotfix Wrapped RPC: avoid USING column ambiguity and keep confirmation independent.
-- Run after 004_weekly_wrapped_rpc.sql. Does not recreate tables, disable RLS, or expose raw rows from other users.
...
begin
  ...
  with week_rows as (
    ...
    select w.*, cb.participants, cb.first_at, cb.clarity, (cb.participants >= 2) as confirmed
    from week_rows w join circle_branch cb on cb.layer = w.layer and cb.signal_id = w.signal_id and cb.consequence_id = w.consequence_id
  )
  ...
  accuracy := case when signals_count > 0 and confirmed_count > 0 then round(confirmed_count::numeric / signals_count * 100)::int else 0 end;
  ...
  with days as (select generate_series(0,6) n), mine as (
    select c.*, (cb.participants >= 2) confirmed from public.contributions c
    join (select layer, signal_id, consequence_id, count(distinct user_id) participants, least(1, .18*sqrt(count(distinct user_id)) + .14*sqrt(count(distinct group_id)) + .10*case when count(distinct evidence)>1 then 1 else .5 end) clarity from public.contributions where theme_id=t_id and circle_id=c_id group by 1,2,3) cb
      on cb.layer = c.layer and cb.signal_id = c.signal_id and cb.consequence_id = c.consequence_id
  ...
  with circle_branch as (select layer, signal_id, consequence_id, count(distinct user_id) participants from public.contributions where theme_id=t_id and circle_id=c_id group by 1,2,3), mine as (
    select c.*, s.label signal_label, coalesce(k.label, c.layer) consequence_label from public.contributions c left join public.catalog_items s on s.id=c.signal_id left join public.catalog_items k on k.id=c.consequence_id join circle_branch cb on cb.layer = c.layer and cb.signal_id = c.signal_id and cb.consequence_id = c.consequence_id
  ...
  if confirmed_count = 0 then
    explain := jsonb_build_array('Пока ни один сигнал не получил независимое подтверждение.', 'Ваши сигналы уже сохранены и ждут откликов круга.', 'Главная тема недели — ' || lower(main_label) || '.');
  else
